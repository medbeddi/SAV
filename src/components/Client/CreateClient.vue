<template>
  <div class="container">
    <div class="card">
      <h1 class="card_title">Ajouter un Client</h1>
      <form class="card_form" @submit.prevent="createClient">
        <div class="input">
          <input type="text" class="input_field" v-model="client.nom" required />
          <label class="input_label">Nom</label>
        </div>
        <div class="input">
          <input type="email" class="input_field" v-model="client.email" required />
          <label class="input_label">Email</label>
        </div>
        <div class="input">
          <input type="tel" class="input_field" v-model="client.telephone" required />
          <label class="input_label">Téléphone</label>
        </div>
        <button type="submit" class="card_button">Ajouter</button>
      </form>
    </div>
...
export default {
  ...
  methods: {
    async createClient() {
      try {
        await ClientService.createClients(this.client);
        this.$router.push({ name: 'Client' }); // Retourner à la liste des clients
      } catch (error) {
