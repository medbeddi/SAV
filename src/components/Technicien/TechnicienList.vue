<template>
  <div class="container">
    <div class="card">
      <h1 class="card_title">Liste des Techniciens</h1>
      <div class="action-bar">
        <button class="card_button" @click="goToCreateTechnicien">
          <i class="fas fa-plus-circle"></i> Ajouter un Technicien
        </button>
      </div>
      <table class="table">
        <thead>
...
export default {
  ...
  methods: {
    ...
    async deleteTechnicien(id) {
      try {
        const confirmed = window.confirm('Êtes-vous sûr de vouloir supprimer ce technicien ?');
        if (!confirmed) return;
